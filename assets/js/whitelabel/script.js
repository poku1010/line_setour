$(document).ready(function() {
  ...
  var $content = $('#content');
    ...
    $content.find('input[type=number].integer').wl_Number({min:1, start:1});
    $content.find('input[type=number].decimal').wl_Number({decimals:2,step:0.5});
    
    //Date and Time fields
    //$content.find('input.date, div.date').wl_Date({value: 'now'});
    $content.find('input.time').wl_Time({});
    
    $content.find('#date_start').wl_Date({});
    $content.find('#date_end').wl_Date({minDate: 1});
    ...
    $content.find('input[type=file]').wl_File({
      ...
      onFileError: function(e, fileobj){
        console.log(e);
        console.log(fileobj);
      }
    });
...
/*
    ...
    $content.find('vote_add_form').wl_Form({
...
*/
    ...
    $content.find('table.chart').wl_Chart({
      onClick: function(value, legend, label, id){
        ...
        $.msg("「" + legend + "」票數：" +value, {header:'提案數據'});
      }
    });
...
/*-----------------------------------------------------------------------------------------------------------------------------*/
    ...
    /*----------------------------------------------------------------------*/
      ...
      $content.find('#post_form').wl_Form({
        ajax: false,
        onError: function(status, error, jqXHR){
        }
      });
      
      $content.find('#vote_form').wl_Form({
        onSuccess: function(data, status){
          if(window.console){
            alert('資料成功送出');
            window.location.href='/admin/vote_manage';
          };
        },
        onError: function(status, error, jqXHR){
      ...
      $content.find('#option_form').wl_Form({
        onSuccess: function(data, status){
          if(window.console){
            alert('資料成功送出');
            window.location.href='/admin/vote_option_manage/'+data;
          };
        },
        onError: function(status, error, jqXHR){
          //$.msg("Callback on Error\nError Status: "+status+"\nError Msg: "+error);
